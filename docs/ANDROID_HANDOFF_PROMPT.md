...
2. leia integralmente:
   ...
   - `android/README.md`;
3. consulte commits e runs recentes de GitHub Actions;
...
O Linux 5.23 é a fonte de verdade funcional, identificado por `linux/BASELINE.md` e pelo SHA-256 autoritativo. A implementação Android é uma reimplementação nativa, não um empacotamento Python/Tkinter.
...
`.github/workflows/android-ci.yml` dispara automaticamente em todo commit/push. Portanto, após cada commit relevante:
1. NÃO peça ao usuário para abrir o GitHub ou disparar o workflow;
...
`workflow_dispatch` existe apenas como fallback. O fluxo normal é **commit/push → CI automática**.
...
Antes de confiar neste resumo, confirme tudo no repositório e na CI.
...
2. estabilizar dependências reais;
...
7. avançar seriada e documentadamente pelos milestones de `docs/ANDROID_MIGRATION_PLAN.md`.
...
- Antes de cada escrita, reconfirme o HEAD se houve intervalo ou possibilidade de avanço concorrente.
- Atualize `docs/CURRENT_STATE.md` ao fechar um gate ou mudar estado relevante.
...
Comece agora consultando o estado REAL do repositório e os runs recentes, informe sucintamente o próximo gate e então prossiga executando-o — não pare apenas no planejamento.
```
